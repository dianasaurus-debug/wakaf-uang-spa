 ...
 * @description
 *
 * Namespace from 'angular-mocks.js' which contains test related code.
 *
 */
angular.mock = {};
 ...
 * @name $httpBackend
 * @description
 * Fake HTTP backend implementation suitable for unit test applications that use the
 * {@link ng.$http $http service}.
 *
 * <div class="alert alert-info">
 * **Note**: For fake HTTP backend implementation suitable for end-to-end test or backend-less
 * development please see {@link ngMockE2E.$httpBackend e2e $httpBackend mock}.
 * </div>
 *
 * During unit test, we want our unit tests to run quickly and have no external dependencies so
 ...
 * the requests and respond with some test data without sending a request to a real server.
 *
 ...
 * The test will pass whether or not the request gets made during test.
 *
 *
 * <table class="table">
 ...
 *     <td>loose (black-box) unit test</td>
 *   </tr>
 *   <tr>
 ...
 * test, the request expectations are evaluated first.
 *
 ...
 * this behavior in unit test, we'd have to create async unit tests, which are hard to write,
 * to follow and to maintain. But neither can the test mock respond synchronously; that would
 * change the execution of the code under test. For this reason, the mock $httpBackend has a
 * `flush()` method, which allows the test to explicitly flush pending requests. This preserves
 * the async api of the backend, while allowing the test to execute synchronously.
 *
 *
 * ## Unit test with mock $httpBackend
 * The following code shows how to setup and use the mock backend when unit test a controller.
 ...
 * Returns instance for unit test (when no arguments specified):
 *   - passing through is disabled
 *   - auto flushing is disabled
 *
 * Returns instance for e2e test (when `$delegate` and `$browser` specified):
...
function MockXhr() {
  ...
  // hack for test $http, $httpBackend
  ...
  // When we are test $httpBackend (inside the AngularJS project) we make partial use of this
 ...
 * @name $controller
 * @description
 * A decorator for {@link ng.$controller} with additional `bindings` parameter, useful when test
 ...
 * @description
 * A service that can be used to create instances of component controllers. Useful for unit-test.
 *
 ...
 *
 * See also the section on {@link guide/component#unit-test-component-controllers unit-test component controllers}
 * in the guide.
 *
 ...
 * @description
 *
 * The `ngMockE2E` is an AngularJS module which contains mocks suitable for end-to-end test.
 ...
 * @module ngMockE2E
 * @description
 * Fake HTTP backend implementation suitable for end-to-end test or backend-less development of
 * applications that use the {@link ng.$http $http service}.
 *
 * <div class="alert alert-info">
 * **Note**: For fake http backend implementation suitable for unit test please see
 * {@link ngMock.$httpBackend unit-test $httpBackend mock}.
 * </div>
 *
 ...
 *
 * As opposed to unit-test, in an end-to-end test scenario or in scenario when an application
 ...
 * test. For this reason the e2e $httpBackend flushes mocked out requests
 ...
 *
 * @example
 * <example name="httpbackend-e2e-test" module="myAppE2E" deps="angular-mocks.js">
 * <file name="app.js">
 ...
 * @module ngMock
 * @description
 * {@link ng.$rootScope.Scope Scope} type decorated with helper methods useful for test. These
...
(function() {
   ...
   * included.
   *
   * It can be used to trigger a native browser event on an element, which is useful for unit test.
   *
   *
  ...
  window.browserTrigger = function browserTrigger(element, eventType, eventData) {
     ...
     * is only here to allow for test where the timeStamp value is
     * read */
